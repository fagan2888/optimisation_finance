...
public class Exercice1_simu {
	public static void solveMe(FileWriter fw, int n, int s, double R, double a)
			throws IOException {
		...
		for (int i = 0; i < n; i++) {
			// b[i] = 150 - randomGeneratorb.nextInt(100); // Les prix initiaux
		...
		try {
			...
			GLPK.glp_set_prob_name(lp, "Minimization de la CVaR");
			...
			for (int i = 2; i <= s + 1; i++) {
				...
				GLPK.glp_set_obj_coef(lp, i, 1 / ((1 - a) * s));
			}
			for (int i = s + 2; i <= 1 + n + s; i++) {
				GLPK.glp_set_col_name(lp, i, "x" + (i - s - 1));
				GLPK.glp_set_col_kind(lp, i, GLPKConstants.GLP_CV);
				GLPK.glp_set_col_bnds(lp, i, GLPKConstants.GLP_DB, -1.0, 1.0);
				GLPK.glp_set_obj_coef(lp, i, 0);
			}

			// Define objective
			GLPK.glp_set_obj_name(lp, "CVaR");
			...
			GLPK.glp_set_row_name(lp, s + 2, "Rendement");
			GLPK.glp_set_row_bnds(lp, s + 2, GLPKConstants.GLP_LO, R, 0.0);
			...
			// set coefs for VaR
			// float matrice[][] = new float[s + 3][s + n + 1];
			int counter = 1;
			...
			for (int i = s + 1; i <= s + 3; i++) {
				...
				counter++;
				// matrice[i - 1][1 - 1] = 0; // matrice[j-1][0]
			}
			...
			for (int i = 1; i <= s; i++) {
				for (int j = s + 2; j <= s + n + 1; j++) {
					GLPK.intArray_setitem(indRows, counter, i);
					GLPK.intArray_setitem(indCols, counter, j);
					float inter = b[j - s - 2] - y[j - s - 2][i - 1];
			...
			for (int j = s + 2; j <= s + n + 1; j++) {
				...
				GLPK.intArray_setitem(indCols, counter, j);
				GLPK.doubleArray_setitem(val, counter, m[j - s - 2]);
				counter++;
				// matrice[s + 1][j - 1] = (float) m[j - s - 2];
			}
			for (int j = s + 2; j <= s + n + 1; j++) {
			...
			if (ret == 0) {
				...
				fw.append("The problem could not be solved");
			}
			// Free memory
